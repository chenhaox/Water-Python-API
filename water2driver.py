...
class Water2Robot():
    ...
    def __init__(self, monitor_server = False):
        """
        ...
        ip = '192.168.10.10'
        port = 31001
        self.__w2_ipad = (ip, port)
        self.__w2_socket = socket.create_connection(self.__w2_ipad,timeout=2)
        print(f"Connection on {self.__w2_ipad}")
    ...
    def move_to_marker(self, target_name, uuid=1, max_continuous_retries=30, distance_tolerance=None,
        ...
        :return:
        """
        command = generate_request_data(uuid, "move", {"marker": target_name,
    ...
    def move_to_location(self, x, y, theta, uuid=1, max_continuous_retries=30, distance_tolerance=None,
        ...
        :return:
        """
        command = generate_request_data(uuid, "move", {"location": f"{x},{y},{theta}",
                                                       "max_continuous_retries": max_continuous_retries,
                                                       "distance_tolerance": distance_tolerance,
                                                       "theta_tolerance": theta_tolerance,
                                                       "angle_offset": angle_offset,
    ...
    def move_to_markers(self, markers: list, uuid=1, max_continuous_retries=5, distance_tolerance=0.5, count=1):
        ...
        command = generate_request_data(uuid, "move", {"markers": ",".join(markers),
                                                       "max_continuous_retries": max_continuous_retries,
                                                       "distance_tolerance": distance_tolerance,
                                                       "count": count,
                                                       })
    ...
    def cancel_move(self, uuid=1):
        ...
        command = generate_request_data(uuid, "move/cancel")
        return self.data_send_recv(command)
    ...
    def robot_status(self, uuid=1):
        ...
        command = generate_request_data(uuid, "robot_status")
        return self.data_send_recv(command)
    ...
    def robot_info(self, uuid=1):
        ...
        command = generate_request_data(uuid, "robot_info")
        return self.data_send_recv(command)
    ...
    def marker_insert(self, name: str, uuid=1, type=0, num=1):
        ...
        command = generate_request_data(uuid, "markers/insert", {"name": name,
                                                                 "type": type,
                                                                 "num": num,
                                                                 })
        return self.data_send_recv(command)
    ...
    def marker_insert_by_pose(self, x: float, y: float, theta: float, name: str, uuid=1, type=0, num=1,
        ...
        command = generate_request_data(uuid, "markers/insert_by_pose", {
            "x": x, "y": y, "theta": theta, "floor": floor,
            "name": name,
            "type": type,
            "num": num,
        })
    ...
    def marker_query_list(self, floor=None, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "markers/query_list", {"floor": floor})
        return self.data_send_recv(command)
    ...
    def marker_query_brief(self, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "markers/query_brief")
        return self.data_send_recv(command)
    ...
    def marker_delete(self, name, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "markers/delete", {"name": name})
        return self.data_send_recv(command)
    ...
    def marker_count(self, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "markers/count")
        return self.data_send_recv(command)
    ...
    def joy_control(self, uuid=1, angular_velocity=0, linear_velocity=0):
        ...
        Details : http://waterdocs.pages.yunjichina.com.cn/user_manual/exports/water_api.html#6.%E6%9C%BA%E5%99%A8%E4%BA%BA%E7%9B%B4%E6%8E%A5%E6%8E%A7%E5%88%B6%E6%8C%87%E4%BB%A4
        :return:
        """
        command = generate_request_data(uuid, "joy_control", {"angular_velocity":angular_velocity,"linear_velocity":linear_velocity})
        return self.data_send_recv(command)
    ...
    def estop(self, flag = False, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "estop", {"flag":"true" if flag else "false"})
        return self.data_send_recv(command)
    ...
    def position_adjust(self, marker:str, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "position_adjust", {"marker":marker})
        return self.data_send_recv(command)
    ...
    def position_adjust_by_pose(self, x: float, y: float, theta: float, floor = None, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "position_adjust_by_pose", {"x": x, "y": y, "theta": theta, "floor":floor })
        return self.data_send_recv(command)

    def request_data(self, topic= "robot_status",  frequency = 2, uuid=1):
        """
        请求server端以一定频率发送topic类型的数据。当请求成功后，server端会以一定频率发送数据给请求的client。
        :param topic: 请求的实时数据类型	 robot_status(请求的实时数据类型)/human_detection(人腿识别模块)/robot_velocity(请求的实时数据类型)
        :param frequency:   发送频率 默认2HZ
        :return:
        """
        command = generate_request_data(uuid, "request_data", {"topic": topic, "frequency":frequency })
        return self.data_send_recv(command)
    ...
    def set_params(self, max_speed = None, max_speed_ratio = None, max_speed_linear = None, max_speed_angular = None, uuid=1):
        ...
        :param max_speed_linear: max_speed_linear	机器人最大直线速度	可选	[0.1, 1.0] (m/s)	小于0.1取0.1，大于1.0取1.0	v0.8.6
        :param max_speed_angular: max_speed_angular	机器人最大角速度	可选	[0.5, 3.5] (rad/s)	小于0.5取0.5，大于3.5取3.5	v0.8.6
        :param uuid:
        :return:
        """
        command = generate_request_data(uuid, "set_params", {"max_speed": max_speed, "max_speed_ratio":max_speed_ratio, "max_speed_linear":max_speed_linear,
    ...
    def get_params(self, uuid=1):
        ...
        """
        command = generate_request_data(uuid, "get_params")
        return self.data_send_recv(command)
    ...
    def wifi_list(self, uuid=1):
        ...
        """
        command = generate_request_data(uuid, "wifi/list")
        return self.data_send_recv(command)

    def wifi_connect(self, SSID, password = None, uuid=1):
        """

        :param SSID: WiFi的SSID	必选	当前的环境WiFi		v0.5.2
        :param password: WiFi密码	可选	SSID对应	如果已经连接过，可以不填	v0.5.2
        :param uuid:
        :return:
        """
    ...
    def wifi_get_active_connection(self, uuid=1):
        ...
        获取机器人当前连接的WiFi的SSID.
        """
        command = generate_request_data(uuid, "wifi/get_active_connection")
        return self.data_send_recv(command)
    ...
    def wifi_info(self, uuid=1):
        ...
        """
        command = generate_request_data(uuid, "wifi/info")
        return self.data_send_recv(command)
    ...
    def wifi_detail_list(self, uuid=1):
        ...
        """
        command = generate_request_data(uuid, "wifi/detail_list")
        return self.data_send_recv(command)
    ...
    def map_list(self, uuid=1):
        ...
        获取机器人中所有的地图名称和楼层。
        """
        command = generate_request_data(uuid, "map/list")

    def map_list_info(self, uuid=1):
        """
        获取机器人中所有的地图的详细信息。
        """
        command = generate_request_data(uuid, "map/list_info")
        return self.data_send_recv(command)
    ...
    def map_set_current_map(self, map_name, floor, uuid=1):
        ...
        :param map_name: map_name	地图名	必选		v0.5.10
        :param floor: floor	楼层	必选		v0.5.2
        :param uuid:
        :return:
        """
        command = generate_request_data(uuid, "map/set_current_map", {"map_name":map_name, "floor":floor})
        return self.data_send_recv(command)
    ...
    def map_get_current_map(self, uuid=1):
        ...
        获取机器人当前地图。
        """
        command = generate_request_data(uuid, "map/get_current_map")
        return self.data_send_recv(command)
    ...
    def shutdown(self, reboot = False, delay = 0, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "shutdown",{
            "reboot": "true" if reboot else "false", "delay": delay
        })
        return self.data_send_recv(command)

    def set_luminance(self, value, uuid=1):
        # value [0-100]
        command = generate_request_data(uuid, "LED/set_luminance", {
            "value":value,
        })
        return self.data_send_recv(command)
    ...
    def diagnosis_get_result(self, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "diagnosis/get_result")
        return self.data_send_recv(command)
    ...
    def get_power_status(self, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "get_power_status")
        return self.data_send_recv(command)
    ...
    def get_planned_path(self, uuid=1):
        ...
        :return:
        """
        command = generate_request_data(uuid, "get_planned_path")
        return self.data_send_recv(command)
    ...
    def make_plan(self, start_x, start_y, start_floor, goal_x, goal_y, goal_floor, uuid=1):
        ...
        """
        command = generate_request_data(uuid, "make_plan", {
            "start_x":start_x, "start_y": start_y, "start_floor": start_floor,
            "goal_x": goal_x, " goal_y": goal_y, " goal_floor": goal_floor
        })
        return self.data_send_recv(command)
